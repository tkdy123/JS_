...
/*
  일급객체의 조건
  1) 변수에 할당 할 수 있다.
  2) 함수의 매개변수로 전달 받는다. 매개변수(Parameter), 인자(Argument)
  3) 함수의 결과로 반환 할 수 있다. 

  일급함수의 조건 => 자바스크립트는 함수를 객체로 취급한다.
  1) 변수에 함수를 할당 할 수 있다.
  2) 함수의 매개변수로 다른 함수를 전달 받는다. 매개변수(Parameter), 인자(Argument)
  3) 함수의 결과로 다른 함수를 반환 할 수 있다. 
*/
...
console.log('newFunction ||',newFunction , 'sampleFunc ||',sampleFunc)
sampleFunc();
...
// 3) 클로저 : 특정 함수를 생성 할 때 선언 된 환경을 기억함
function orderSet(){
  let side = '감자튀김';
  let beverage = '콜라';
...
function User(id, pwd) { // 함수의 이름 대문자로 시작
  // 필드 this.OO;
  this.id = id; 
  this.pwd = pwd;
...
let msg = testFunc('First', 'Wellcome!');
console.log(msg);
...
// 3. 매개변수가 하나 밖에 없는 경우 4. 실행하고자 하는 명령어가 하나인 경우
...
// Rest 매개변수 : 선언된 매개변수 외 추가로 들어오는 값을 저장, 배열
// 매개변수 기본값(ex || plus(x = 0, y = 0)) : 주어진 인자값이 없는 경우 사용 할 값
